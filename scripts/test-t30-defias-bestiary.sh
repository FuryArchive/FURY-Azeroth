...
grep -Fq 'event.correlationKey == Defias::GraphKey' "${SERVICE}"
grep -Fq 'defias.bestiary.entity.participated' "${ROOT}/modules/mod-fury/src/content/defias/DefiasParticipation.cpp"
grep -Fq 'defias:bestiary-participation:v1:' "${ROOT}/modules/mod-fury/src/content/defias/DefiasParticipation.cpp"
grep -Fq 'director.run.resolved' "${SERVICE}"
grep -Fq '*run->resolvedEventId != source.id' "${SERVICE}"
