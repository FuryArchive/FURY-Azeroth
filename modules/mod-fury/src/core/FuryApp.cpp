...
App::App()
    : _households(_householdRepository),
      ...
      _defiasRecovery(_director, _directorRepository, _livingWorld, _events),
...
{
    if (!_defiasRecovery.Reconcile())
    {
        LOG_ERROR(
            "server.loading",
            "[FURY Recovery] Defias reconciliation reported an unresolved recovery action; it will retry on the next reconcile tick.");
    }
}
}
