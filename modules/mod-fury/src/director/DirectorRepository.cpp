...
DirectorRun ReadRun(Field* fields)
{
    ...
    run.participationScore = fields[7].Get<uint32>();
    run.startedEventId = fields[8].Get<EventId>();
    run.lastEventId = fields[9].Get<EventId>();

    if (!fields[10].IsNull())
        run.resolvedEventId = fields[10].Get<EventId>();

    if (!fields[11].IsNull())
        run.outcomeKey = fields[11].Get<std::string>();

    run.revision = fields[12].Get<uint64>();
    return run;
}
}
    ...
    EventId sourceEventId) const
{
    ...
    stmt->SetData(3, householdId);
    stmt->SetData(4, expectedRevision);
    stmt->SetData(5, externalRuntimeId);
    FuryDatabase.Execute(stmt);
}
