    ...
    uint16 optionOrdinal) const
{
    ...
    if (persisted->optionOrdinal != optionOrdinal)
    {
        return {
    ...
    return {ProfessionOrderStartOutcome::Started, persisted};
}
...
bool ProfessionOrderService::Handle(FuryEvent const& event)
{
    ...
    for (ProfessionOrderInstance const& instance : matches)
    {
        ...
        if (progressed->progressCount < progressed->requiredCount)
            continue;

        std::optional<EventId> completionEventId =
            EmitCompletedEvent(event, *progressed);
    ...
    ProfessionOrderInstance const& instance) const
{
    ...
    event.payloadJson = Acore::StringFormat(
        ...
        "\"required_count\":{},\"source_event_id\":{}}}",
        instance.id,
        instance.orderKey,
